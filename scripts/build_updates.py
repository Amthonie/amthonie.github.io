#!/usr/bin/env python3
"""
Build the /updates/ page (and the homepage teasers) from markdown sources.

Source of truth: content/updates/*.md — one file per update, each starting with
...
Outputs (all committed as build artifacts, exactly like styles.css):
  - updates/index.html   full list, newest first, served at /updates/
  - index.html           teaser cards injected between the
                         <!-- UPDATES:START --> / <!-- UPDATES:END --> markers
  - sitemap.xml          /updates/ entry kept in sync (lastmod = newest post)

Run from the repo root:  python3 scripts/build_updates.py
...
CONTENT_DIR = ROOT / "content" / "updates"
UPDATES_PAGE = ROOT / "updates" / "index.html"
INDEX_PAGE = ROOT / "index.html"
SITEMAP = ROOT / "sitemap.xml"

SITE = "https://amthonie.nl"
TEASERS_ON_HOME = 2  # how many of the newest updates to show on the homepage
...
def render_articles(posts: list[dict]) -> str:
    if not posts:
        return (
            '<p class="text-stone-600 dark:text-stone-400">No updates yet — '
            "check back soon.</p>"
        )
...
def render_teasers(posts: list[dict]) -> str:
    if not posts:
        return (
            ...
            "Nothing here yet — updates will appear as I post them.</p>"
        )
...
<head>
    ...
    <meta property="og:title" content="Amthonie | Updates"/>
...
<main class="flex min-h-screen flex-col items-center px-2.5 md:px-6 pt-6 md:pt-12 pb-12 md:pb-24">
    ...
    <a
        ...
        <img
                ...
                class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 aspect-square h-1/2 rounded-full object-cover bg-[#b0c7bd] ring-4 ring-[#b0c7bd] shadow-md"
                draggable="false"
        />
    </a>
    ...
    <section
        ...
        <div class="flex items-center justify-between gap-4">
            ...
            <a href="../"
               class="inline-flex items-center gap-1.5 rounded-full border border-black/10 bg-black/5 px-4 py-2 text-sm font-medium text-stone-700 transition hover:bg-black/10 dark:border-white/15 dark:bg-white/10 dark:text-stone-200 dark:hover:bg-white/20">
...
"""
    UPDATES_PAGE.parent.mkdir(parents=True, exist_ok=True)
    UPDATES_PAGE.write_text(page, encoding="utf-8")
    print(f"wrote {UPDATES_PAGE.relative_to(ROOT)} ({len(posts)} update(s))")


def inject_home_teasers(posts: list[dict]) -> None:
    marker = re.compile(
        r"(<!-- UPDATES:START -->).*?(<!-- UPDATES:END -->)", re.DOTALL
    )
    text = INDEX_PAGE.read_text(encoding="utf-8")
    if not marker.search(text):
        raise SystemExit(
            "index.html is missing the <!-- UPDATES:START --> / "
            "<!-- UPDATES:END --> markers"
        )
...
def update_sitemap(posts: list[dict]) -> None:
    """Keep the sitemap in sync with the newest update.

    Ensures a /updates/ entry exists, and bumps both /updates/ and the home
    page lastmod to the newest post's date — the home page changes because its
    teaser cards do. The gallery entry is left untouched (updates don't affect
    it).
    """
    if not posts:
        return
    lastmod = f"{posts[0]['date']:%Y-%m-%d}"
    updates_loc = f"{SITE}/updates/"
    home_loc = f"{SITE}/"
    text = SITEMAP.read_text(encoding="utf-8")

    if updates_loc in text:
        text = _set_lastmod(text, updates_loc, lastmod)
    else:
        entry = (
            "    <url>\n"
            f"        <loc>{updates_loc}</loc>\n"
    ...
    print(f"sitemap.xml: home + /updates/ lastmod {lastmod}")


def main() -> int:
    posts = load_posts()
    build_updates_page(posts)
